...
public class GetAllPostDataTask extends AsyncTask<NBTopicData, Void, Void> {
    ...
    private HashMap<Long, NBPostData> nbPostDataHashMap;
    ...
    @Override
    protected Void doInBackground(final NBTopicData... nbTopicData) {
        if (nbTopicData[0] != null) {
            for (int i = 0; i < nbTopicData[0].getPostUpdateData().size(); i++) {
                new NBFirebasePostData(nbTopicData[0].getPostUpdateData().get(i).getId(), 1, OustAppState.getInstance().getActiveUser().getStudentKey(), "SINGLETON") {
                    @Override
            ...
            OustSdkTools.sendSentryException(e);
                        ...
                        if (nbPostDataHashMap.size() == nbTopicData[0].getPostUpdateData().size()) {
                            List<NBPostData> postDataList = new ArrayList<>(nbPostDataHashMap.values());
                            Log.d("GetAllPostDataTask", "postDataListList: " + postDataList.size());
                        ...
                        Log.d("GetAllPostDataTask", "postDataListCount: " + postViewDataList.size());
                    }
                };
            }
        }
    ...
    public Comparator<NBPostData> nbPostDateComparator = new Comparator<NBPostData>() {
        public int compare(NBPostData s1, NBPostData s2) {
