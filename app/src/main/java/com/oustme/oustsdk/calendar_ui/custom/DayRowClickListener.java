...
import com.oustme.oustsdk.calendar_ui.custom.utils.CalendarUtils;
...
public class DayRowClickListener implements AdapterView.OnItemClickListener {
    ...
    public DayRowClickListener(OustCalendarPageAdapter calendarPageAdapter, CalendarProperties calendarProperties, int pageMonth) {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
        ...
        switch (mCalendarProperties.getCalendarType()) {
            case OustCalendarView.SINGLE_DAY_PICKER:
                selectOneDay(view, day);
                break;

            case OustCalendarView.MULTIPLE_DAYS_PICKER:
                selectManyDays(view, day);
                break;

            case OustCalendarView.BETWEEN_PICKER:
                selectRange(view, day);
                break;

            case OustCalendarView.STANDARD:
    ...
    private void reverseUnselectedColor(SelectedDay selectedDay) {
        if(OustStaticVariableHandling.getInstance().getEventDayArrayList()!=null&& OustStaticVariableHandling.getInstance().getEventDayArrayList().size()!=0){
            if(OustStaticVariableHandling.getInstance().getEventDayArrayList().contains(selectedDay)){
