...
public class TaskHashMapContestAdapter extends RecyclerView.Adapter<TaskHashMapContestAdapter.ViewHolder> implements Filterable {
    ...
    int type;
    TaskHashMapContestAdapter.ValueFilter valueFilter;
    ArrayList<String> keyList;
    TaskListContestAdapter adapter;
    ...
    public void setTaskModuleHashMap(Map<String, ArrayList<CommonLandingData>> taskModuleHashMap) {
        ...
        this.keyList = new ArrayList<>(taskModuleHashMap.keySet());
        OustStaticVariableHandling.getInstance().setTaskContextModuleHashMap(taskModuleHashMap);
    }

    public Map<String, ArrayList<CommonLandingData>> getList() {
        if (tempTaskModuleHashMap != null) {
            return tempTaskModuleHashMap;
        } else {
            return new TreeMap<>();
        }
    }
    ...
    @NonNull
    @Override
    public TaskHashMapContestAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.common_adapter_module, parent, false);
        TaskHashMapContestAdapter.ViewHolder viewHolder = new TaskHashMapContestAdapter.ViewHolder(v);
        viewHolder.setIsRecyclable(false);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull TaskHashMapContestAdapter.ViewHolder holder, int position) {
        ...
        try {
            ...
            if (dateKey != null && !dateKey.isEmpty()) {
                ...
                adapter = new TaskListContestAdapter();
    ...
    public Filter getFilter() {
        if (valueFilter == null) {
            valueFilter = new TaskHashMapContestAdapter.ValueFilter();
        }
        return valueFilter;
    }
    ...
    private class ValueFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                if (OustStaticVariableHandling.getInstance().getTaskContextModuleHashMap() != null && OustStaticVariableHandling.getInstance().getTaskContextModuleHashMap().size() != 0) {
                    for (String data : OustStaticVariableHandling.getInstance().getTaskContextModuleHashMap().keySet()) {
