...
public class TaskHashMapModuleAdapter extends RecyclerView.Adapter<TaskHashMapModuleAdapter.ViewHolder> implements Filterable {
    ...
    public void setTaskRecyclerAdapter(Map<String, ArrayList<CommonLandingData>> taskModuleHashMap,
                                       Context context, int type, boolean isAscending) {
    ...
    public void setTaskModuleHashMap(Map<String, ArrayList<CommonLandingData>> taskModuleHashMap) {
        OustStaticVariableHandling.getInstance().setTaskAssessmentModuleHashMap(taskModuleHashMap);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        try {
            ...
            if (dateKey != null && !dateKey.isEmpty()) {
                ...
                TaskListModuleAdapter adapter = new TaskListModuleAdapter();
    ...
    public int getItemCount() {
        ...
        if (!isAscending) {
            this.keyList = new ArrayList<>(tempTaskModuleHashMap.keySet());
            Collections.sort(keyList, CommonLandingData.ascendingSortForKey);
        } else {
            this.keyList = new ArrayList<>(tempTaskModuleHashMap.keySet());
            Collections.sort(keyList, CommonLandingData.descendingSortForKey);
        }
    ...
    private class ValueFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                if (OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap() != null && OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap().size() != 0) {
                    for (String data : OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap().keySet()) {
                        ArrayList<CommonLandingData> commonLandingData = OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap().get(data);
                ...
                results.count = OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap().size();
                results.values = OustStaticVariableHandling.getInstance().getTaskAssessmentModuleHashMap();
            }
            return results;
        }
