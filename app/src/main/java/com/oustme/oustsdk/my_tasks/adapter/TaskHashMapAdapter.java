...
public class TaskHashMapAdapter extends RecyclerView.Adapter<TaskHashMapAdapter.ViewHolder> implements Filterable {
    ...
    public void setTaskRecyclerAdapter(Map<String, ArrayList<CommonLandingData>> taskModuleHashMap,
        ...
        this.filterCourseModuleHashMap = taskModuleHashMap;
    ...
    public void setTaskModuleHashMap(Map<String, ArrayList<CommonLandingData>> taskModuleHashMap) {
        OustStaticVariableHandling.getInstance().setTaskModuleHashMap(taskModuleHashMap);
    }
    ...
    public void filterInvoke() {
        try {
            tempTaskModuleHashMap = filterCourseModuleHashMap;
            notifyDataSetChanged();
        } catch (Exception e) {
            e.printStackTrace();
            OustSdkTools.sendSentryException(e);
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                if (OustStaticVariableHandling.getInstance().getTaskModuleHashMap() != null && OustStaticVariableHandling.getInstance().getTaskModuleHashMap().size() != 0) {
                    for (String data : OustStaticVariableHandling.getInstance().getTaskModuleHashMap().keySet()) {
                        ArrayList<CommonLandingData> commonLandingData = OustStaticVariableHandling.getInstance().getTaskModuleHashMap().get(data);
                        if (commonLandingData != null && commonLandingData.size() > 0) {
                            ...
                            for (CommonLandingData landingData : commonLandingData) {
                                if (landingData.getName() != null && landingData.getType() != null && (landingData.getType().equalsIgnoreCase("course") ||
                                        landingData.getType().equalsIgnoreCase("CLASSROOM") || landingData.getType().equalsIgnoreCase("WEBINAR"))) {
                ...
                results.count = filterCourseModuleHashMap.size();
                results.values = filterCourseModuleHashMap;
            }
            return results;
        }
