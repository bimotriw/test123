...
public class TaskListAdapter extends RecyclerView.Adapter<TaskListAdapter.ViewHolder> {
    ...
    private void getData(CommonLandingData task, ViewHolder holder, int position) {
        try {
            if (task != null) {
                ...
                if (task.getIcon() != null && !task.getIcon().isEmpty()) {
                    ...
                    Picasso.get().load(imageUrl).resize(200, 200).error(R.drawable.course_thumbnail).into(holder.iv_banner);
//                    Glide.with(context).load(imageUrl).diskCacheStrategy(DiskCacheStrategy.ALL).into(holder.iv_banner);
                } else {
                    Glide.with(context).load(R.drawable.course_thumbnail).placeholder(R.drawable.course_thumbnail).diskCacheStrategy(DiskCacheStrategy.ALL).into(holder.iv_banner);
                }
                ...
                holder.root_common_lay.setOnClickListener(v -> {
                    ...
                    scaleDown.addListener(new Animator.AnimatorListener() {
                        ...
                        public void onAnimationEnd(Animator animator) {
                            launchActivity(task, holder.getAbsoluteAdapterPosition());
                        }
    ...
    private void launchActivity(CommonLandingData commonLandingData, int pos) {
        ...
        if ((commonLandingData.getType() != null) && (commonLandingData.getType().contains("ASSESSMENT"))) {
            ...
            BranchTools.gotoCalendarPage(meetingId);
        } else {
            ...
            taskCourseIntent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_CLEAR_TOP);
