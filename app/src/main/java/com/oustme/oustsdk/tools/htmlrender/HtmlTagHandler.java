...
public class HtmlTagHandler implements Html.TagHandler {
    ...
    public static final String BOLD_STYLE = "HTML_TEXTVIEW_ESCAPED_B_TAG";
    public static final String ITALIC_STYLE = "HTML_TEXTVIEW_ESCAPED_I_TAG";
    public static final String UNDERLINE_STYLE = "HTML_TEXTVIEW_ESCAPED_U_TAG";
    ...
    String overrideTags(@Nullable String html){
        ...
        html = html.replace("</li>", "</" + LIST_ITEM + ">");
        html = html.replace("</b>", "</"+BOLD_STYLE+">");

        return html;
    }

    Stack<String> boldStack = new Stack<>();
    ...
    public void handleTag(final boolean opening, final String tag, Editable output, final XMLReader xmlReader) {
        ...
        if (opening) {
            ...
            if(tag.equalsIgnoreCase(BOLD_STYLE))
            {
                boldStack.push(tag);
                start(output, new B());
            }
...
//                if(output.length()>0){
//                    while (output.charAt(output.length() - 2) == '\n'||output.charAt(output.length() - 2) == '\t') {
            ...
            if(tag.equalsIgnoreCase(BOLD_STYLE)){
                ...
                if (!lists.isEmpty()) {
                    if (lists.peek().equalsIgnoreCase(UNORDERED_LIST)) {
                        ...
                        OustBulletSpan oustBulletSpan=new OustBulletSpan();
//                        BulletSpan newBullet = new BulletSpan(bulletMargin);
                        ...
                        NumberSpan numberSpan = new NumberSpan(olNextIndex.lastElement() - 1);//mTextPaint
